...
// CountByRoleGroupID returns how many super admins reference the given role group.
func (r *SuperAdminRepository) CountByRoleGroupID(ctx context.Context, roleGroupID primitive.ObjectID) (int64, error) {
	return r.coll.CountDocuments(ctx, bson.D{{Key: "role_group_ids", Value: roleGroupID}})
}

// PullRoleGroupFromAll removes the role group reference from all super admins.
func (r *SuperAdminRepository) PullRoleGroupFromAll(ctx context.Context, roleGroupID primitive.ObjectID) error {
	_, err := r.coll.UpdateMany(ctx, bson.D{{Key: "role_group_ids", Value: roleGroupID}}, bson.D{{Key: "$pull", Value: bson.D{{Key: "role_group_ids", Value: roleGroupID}}}})
	return err
}
