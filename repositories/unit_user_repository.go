...
// PullRoleGroupFromAll removes the given role group from all users in the unit.
func (r *UnitUserRepository) PullRoleGroupFromAll(ctx context.Context, unitID, roleGroupID primitive.ObjectID) error {
	_, err := r.coll.UpdateMany(ctx,
		bson.D{{Key: "unit_id", Value: unitID}},
		bson.D{{Key: "$pull", Value: bson.D{{Key: "role_group_ids", Value: roleGroupID}}}},
	)
	return err
}
