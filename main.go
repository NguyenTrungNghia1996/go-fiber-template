...
func main() {
	...
	// Kết nối Postgres một lần duy nhất
	config.ConnectDB()
	...
	saRepo := repositories.NewSuperAdminRepository(config.DB)
	...
	unitServicePackageRegistrationRepo := repositories.NewUnitServicePackageRegistrationRepository(config.DB)
	...
	// Units feature
	unitRepo := repositories.NewUnitRepository(config.DB, unitServicePackageRegistrationRepo)
